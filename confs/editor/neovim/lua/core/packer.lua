...
Global.put('plug', function (props, config)
  if config then
    props.config = config
  end
...
function this:boostrap()
  if fn.empty(fn.glob(this.install_path)) > 0 then
    ...
    this.installed = fn.system({'git', 'clone', '--depth', '1', 'https://github.com/wbthomason/packer.nvim', this.install_path}) 
...
function this:init()
  local user = require('user.config')
    ...
    use({ user.theme.re, as = user.theme.as, config = function()
      local user = require('user.config')
